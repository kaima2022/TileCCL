...
def fig6_collective_comm_only():
    ...
    for collective, title in op_order:
        ...
        if any(point is None for point in small_points):
            continue
        latency_collectives.append((collective, title, series, small_points))
    ...
    if allreduce_series:
        candidate_points = [
            _collective_point_at_size(allreduce_series, size)
            for size in allreduce_large_sizes
        ]
        if all(point is not None for point in candidate_points):
            sweep_points = candidate_points

    bottom_panel_count = int(bool(anchor_points)) + int(bool(sweep_points))
    n_latency = max(len(latency_collectives), 1)
    ncols = max(n_latency, bottom_panel_count, 1)
    fig_width = max(9.0, 3.4 * ncols + 1.4)
    ...
    legend_handles = None
    legend_labels = None
    if len(latency_collectives) == 1:
        latency_axes = [fig.add_subplot(grid[0, :])]
    else:
        for idx in range(len(latency_collectives)):
            sharey = latency_axes[0] if latency_axes else None
            latency_axes.append(fig.add_subplot(grid[0, idx], sharey=sharey))

    for idx, ((collective, title, _series, small_points), ax) in enumerate(
        zip(latency_collectives, latency_axes)
    ):
    ...
    if bottom_panel_count == 1:
        bottom_axes.append(fig.add_subplot(grid[1, :]))
    elif bottom_panel_count == 2:
        split = max(1, ncols // 2)
        bottom_axes.append(fig.add_subplot(grid[1, :split]))
        bottom_axes.append(fig.add_subplot(grid[1, split:]))

    bottom_iter = iter(bottom_axes)
    if anchor_points:
        anchor_ax = next(bottom_iter)
    ...
    omitted_collectives = (COLLECTIVE_PAYLOAD.get("environment") or {}).get("omitted_collectives", [])
    ...
    if omitted_collectives:
        fig.text(
            0.5,
            0.955,
            "Partial dataset: showing only collectives that completed in the current rerun",
            ha="center",
            va="top",
            fontsize=8.2,
            color="#666666",
