...
tncc.ops.build_gemm_allgather_plan(...)
tncc.ops.build_allreduce_plan(...)
```
...
| 内容 | 路径 |
|------|------|
| 总状态文档 | `docs/TNCC现状流程_修订版.md` |
| Benchmark 摘要 | `docs/generated/benchmark_runtime_summary.md` |
| Runtime support matrix | `tncc/support.py` |
| 高层 ops | `tncc/ops.py` |
| 内存子系统 | `tncc/memory/` |
| 通信原语 | `tncc/primitives/` |
| Overlap patterns | `tncc/patterns/` |
| 同步原语 | `tncc/sync/` |
| 后端抽象 | `tncc/backends/` |
| 工具/feature gates | `tncc/utils/` |
| GEMM 内核 | `tncc/kernels/` |

## 下一步

1. 内存底座收口：推进到 canonical `segmented peer import/map` + `fd passing / DMA-BUF` 零拷贝。
2. 收缩公共语义面：所有默认路径通过 `tncc.ops.*`，减少 pattern 细节暴露。
