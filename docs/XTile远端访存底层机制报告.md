...
- XTile 既然不调用 `NVSHMEM`，那 `tl.load` / `tl.store` 访问远端 GPU 内存时，底层到底走什么路径。
...
- XTile 当前远端访存路径不是 `NVSHMEM`，也不是 `NCCL`。
- XTile 的核心机制是：先把远端 GPU heap 映射成“当前 rank 可解引用的 GPU 虚拟地址”，再由 Triton 的 `tl.load` / `tl.store` 对该地址发普通 global memory 读写。
- 单进程多卡时，走的是 `CUDA peer access + UVA` 的直接 peer pointer。
- 多进程时，默认走的是 `CUDA IPC handle` 映射后的远端指针；XTile 当前默认验证的 transport 是 `ctypes_ipc`。
- 物理链路取决于机器拓扑：
...
2. 一个把 XTile 的 `translate_ptr` 内联进去的 kernel
...
把 XTile 的 `translate_ptr` 内联进最小 kernel 后，TTIR 中可以直接看到如下链路：
...
换句话说，XTile 的远端访存是“对远端映射地址发普通 global 访存”，不是“调用 NVSHMEM 的 put/get”。

## 一句话答案

XTile 这里走的不是 `NVSHMEM`，也不是 `GDR`；更准确地说，它走的是：
...
XTile 的 collective 原语在模块说明里已经写明：

- 位置：`xtile/primitives/collectives.py`
...
远端访问的核心在 [`xtile/memory/translation.py`](../xtile/memory/translation.py)。
...
也就是说，XTile 并没有在这里调用一个“远端通信 runtime API”。它做的是：

- 先把地址翻译成远端映射地址
- 然后像访问本地 global memory 一样访问它

这也是为什么 XTile 的远端访问能保持在 Triton 编译器可见的 IR 内，而不是掉进一个外部黑盒调用。
...
单进程路径在 [`xtile/memory/symmetric_heap.py`](../xtile/memory/symmetric_heap.py) 的 `create_all()`。
...
多进程路径在 [`xtile/memory/symmetric_heap.py`](../xtile/memory/symmetric_heap.py) 的 `_setup_multiprocess()`。
...
- [`xtile/memory/allocators.py`](../xtile/memory/allocators.py)
...
逻辑上，XTile kernel 只知道自己在访问一个 peer-accessible / IPC-mapped GPU 地址。
...
这说明当前这台机器上，`GPU0 <-> GPU1` 之间是 `NV12`，也就是 bonded NVLink 连接；因此 XTile 的 peer 远端访存物理上会走 NVLink，而不是普通 PCIe 路径。
...
从代码库看，当前 `xtile/` 实现路径里没有 `NVSHMEM` runtime 接入；collective 与 remote access 的关键路径都建立在：
...
XTile 不是 library call 驱动的数据面；它更接近：
...
- 链路空闲，不等于协议会自己推进
- 对 XTile 这种 kernel-driven remote access 模型来说，`SM progress` 和 `interconnect bandwidth` 同样重要
...
因此 XTile 当前路径对 `SM` 干扰敏感，不是偶然现象，而是机制上可以解释的结果。
...
> XTile 当前的远端访存机制，是在 host 侧先通过 CUDA peer access 或 CUDA IPC 把对端 symmetric heap 建立为本 rank 可访问的 GPU 虚拟地址，再在 Triton kernel 内通过 `translate_ptr` 做基址平移，并由普通 `tl.load` / `tl.store` lower 成 `ld.global` / `st.global` 去访问该远端地址。当前机器 `GPU0 <-> GPU1 = NV12`，所以实际物理链路是 NVLink P2P，而不是 GPUDirect RDMA。

## 容易说错的几句话

下面这些说法不够准确：

- “XTile 的 `tl.load/store` 底层是 NVSHMEM”
- “XTile 这里走的是 GDR”
...
- “XTile 走的是 peer access / CUDA IPC 映射后的远端 GPU 地址直接解引用”
- “如果说 GDR，必须区分 GPUDirect P2P 和 GPUDirect RDMA；这里不是 RDMA”
- “XTile 通信是 kernel-driven 的 global memory remote access，既依赖链路，也依赖 SM progress”

## 对工程判断的意义

这份机制分析直接决定后续优化方向。

如果要继续把 XTile 做成可维护、可解释、可追 benchmark 的工业级实现，重点不应只盯：
