...
【修订】原文把 TileScale 当前用户接口写成了 `T.scatter()`。按当前 repo 能直接找到的 public API，更接近 `T.putmem_nbi_block(...)`、`T.put_block(...)`、`T.get_block(...)`、`T.wait_eq(...)`、`T.barrier_all()` / `T.sync_all()` 这类 distributed primitives。普通 `T.copy(src, dst)` 仍然存在，但我没有在当前 repo 里找到公开 `T.scatter()` 或 `T.copy(..., dst=...)` 这种分布式用户接口；下面改成与 examples 和 lowering 一致的写法。
...
**关键点（修订）**：当前 repo 里能直接找到的是较低层的 distributed/NVSHMEM primitives。`T.scatter()` 未见公开接口；普通 `T.copy(src, dst)` 也不是我在当前 repo 中能证实的 remote-copy 用户接口。
...
"编译到 NVSHMEM / distributed extern"更准确地说，并不是所有 distributed primitive 都会变成同一种 NVSHMEM 调用。当前 repo 至少能明确看到两条路径：
...
**关键点（修订）**：概念上，Iris 的核心增量就是“远端 store / put 的那一段循环”，没有额外的 distributed compiler layer，也没有外部 SHMEM 库参与 device-side 远端访问。但当前 repo 的 examples 不止这 5 行，还包含 `global_offset`、mask、本地 fast path、可选 timestamp 等脚手架。
...
当前 released repo 里没有直接公开 `block_channel` 这类对象，因此也没有可直接指认的 `shape_mapping` / `rank_mapping` / `channel_mapping` 用户级 API。能直接对应到仓库里的，是 `distributed.wait`、`distributed.consume_token`、`distributed.symm_at`、`distributed.notify` 这些低层 op；mapping 这一节应理解为论文里的概念模型，而不是当前公开代码接口。
...
更准确地说，这类调用会形成比较保守的优化边界。当前仓库代码没有提供足够高层语义，让上层 pass 把它像普通 `tt.load` / `tt.store` 那样分析。
...
这意味着它不会额外引入 extern / inline-asm 边界。至于 LICM、向量化、重排等具体优化是否真的发生，要看当下 Triton / LLVM pass；当前仓库里我没有看到专门针对跨 GPU `tt.store` 的新 pass。
...
全透明（Iris/XTile）：
  你跟编译器说"把包裹放到架子上"。
  在 IR 层，它跟放到本地架子上属于同一种 load/store 操作家族。
...
【新增核对】当前仓库代码能直接证明的是“IR 更透明”，不能直接从仓库里证明“已经因为这件事拿到了多少现成性能收益”。
