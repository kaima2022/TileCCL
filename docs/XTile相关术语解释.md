...
这份文档专门解释 XTile 里最容易混淆的一批术语，尤其是：
...
本文只讲 XTile 当前代码里的专业含义，不讲泛泛而谈的分布式术语。
...
- 这次 XTile 上下文里，实际被纳入同一个通信/执行组的 rank 数

例子：

- 机器有 8 张 GPU，但这次只用前 2 张跑 XTile
...
XTile 当前在 pattern contract 里关心的布局主要是：
...
XTile 的 `PatternExecutionSpec` 里有：
...
在 XTile 的执行合同里，`output` 不只是“这是个输出”，还会记录：
...
在你当前看到的 XTile 代码里，最常见的是：

- `storage_kind = "symmetric"`

它表示：

- 这个张量放在 XTile 的 symmetric heap 里
...
- 不是任意普通 `torch.cuda.Tensor` 都天然适合做 XTile 的远端对称翻译
- XTile 当前很多远端操作默认要求 tensor 在 symmetric heap 里
...
`scatter` 在 XTile 这里，不要简单理解成“随便发数据”，它是有明确方向和语义的。
...
XTile 当前代码对 mixed layout 是有限支持的，不是所有组合都开放。
...
单进程多卡时，XTile 通常用这条路径建立 `heap_bases`。
...
`translate_ptr` 是 XTile 远端访问的核心函数。
...
例如 XTile 里的 `GemmAllScatterPlan`：
