...
所以，从编译器视角看，TNCC 当前通信表达不是“调用一个通信 runtime API”，而是“翻译后的远端地址 + 标准 Triton memory op”。
...
- [`tncc/backends/hip.py`](../tncc/backends/hip.py)

硬件检测也只在 `cuda` / `hip` 两类里选：

- [`tncc/backends/__init__.py`](../tncc/backends/__init__.py)
...
- 设计目标 / 代码路径：
  - README 与 `tncc/__init__.py` 都把目标写成 NVIDIA + AMD
- 当前公开验证范围：
  - 仓库中的真实 multiprocess remote-access 矩阵当前只公开证明了更小的 surface

已有文档已明确写出当前边界：

- [`docs/TNCC现状流程_修订版.md`](./TNCC现状流程_修订版.md)
...
- [`tncc/support.py`](../tncc/support.py)
...
| 是否依赖外部通信库数据面 | 否 |
| 理论硬件范围 | Triton + TNCC backend 可覆盖的 NVIDIA / AMD GPU |
| 是否对所有 GPU 适用 | 否 |
...
> TNCC 当前的 tile 通信不是独立通信指令或黑盒 runtime，而是基于 Triton 的远端地址翻译加标准 `tl.load` / `tl.store` / `tl.atomic_*`。因此它并不对“所有 GPU”通用，而是面向 Triton 支持且已由 TNCC backend 接入的 NVIDIA / AMD GPU；其中当前公开验证通过的 multiprocess device-side surface 仅为 `world_size=2 + ctypes_ipc`。

## 延伸阅读

- 详细机制：[`docs/TNCC远端访存底层机制报告.md`](./TNCC远端访存底层机制报告.md)
- 当前流程与边界：[`docs/TNCC现状流程_修订版.md`](./TNCC现状流程_修订版.md)
