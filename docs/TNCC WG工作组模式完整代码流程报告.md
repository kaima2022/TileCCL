# TNCC (Tile Native Collective Communication) WG 工作组模式：从上到下完整代码流程报告
...
User Code
    ...
    v
+---------------------------+
| Layer 0: tncc.ops        |  <-- gemm_allscatter() 公共入口
+---------------------------+
...
User Code
  |
  v
tncc.init_local(world_size, heap_size)
  |
  v
...
用户通过 `tncc.ops.gemm_allscatter()` 发起一次 GEMM + AllScatter 融合操作：
...
for ctx in ctxs:
    ...
    tncc.ops.gemm_allscatter(A, B, C, ctx=ctx, pattern="wg_spec")
```
...
```python
# tncc/patterns/contracts.py:92-229
...
### 3. Pattern 执行 (tncc/patterns/wg_specialized.py)

`WGSpecializedPattern.execute()` 是 WG 模式的核心调度器：

```python
# tncc/patterns/wg_specialized.py:127-202
...
```python
# tncc/patterns/wg_specialized.py:83-99
...
### 4. Triton 内核 (tncc/patterns/wg_specialized.py)
...
### 5. Scatter Helper (tncc/patterns/_helpers.py)

`scatter_tile_to_peer()` 是所有 pattern 共享的核心通信原语：

```python
# tncc/patterns/_helpers.py:18-91
@triton.jit
...
### 6. 指针翻译 (tncc/memory/translation.py)

`translate_ptr` 是 TNCC 最关键的 5 条指令：

```python
# tncc/memory/translation.py:38-99
@triton.jit
...
### 7. 同步原语 (tncc/sync/primitives.py)
...
```python
# tncc/memory/symmetric_heap.py (简化)
@classmethod
...
```python
# tncc/patterns/auto_select.py (简化决策树)
...
| 层级 | 模块 | 职责 | 核心类/函数 |
|------|------|------|------------|
| L0 - 公共 API | `tncc/ops.py` | 用户入口，contract 解析，plan 构建 | `gemm_allscatter()`, `GemmAllScatterPlan` |
| L1 - Pattern | `tncc/patterns/wg_specialized.py` | SM 分配，kernel launch，tile 调度 | `WGSpecializedPattern.execute()` |
| L2 - Kernel | 同上 (static method) | Compute/Comm worker 并行，GEMM + scatter | `_wg_specialized_kernel` |
| L3 - Scatter | `tncc/patterns/_helpers.py` | 列映射，远端写入 | `scatter_tile_to_peer()` |
| L4 - 翻译 | `tncc/memory/translation.py` | 对称堆指针翻译 (5 条指令) | `translate_ptr()` |
| L5 - 同步 | `tncc/sync/primitives.py` | Tile 级 release-acquire 信号 | `tile_signal()`, `tile_wait()` |
| L6 - Contract | `tncc/patterns/contracts.py` | Shape/layout 标准化 | `PatternExecutionSpec` |
| L7 - Context | `tncc/__init__.py` | Runtime 初始化，堆管理 | `TNCCContext`, `init_local()` |
| L8 - Memory | `tncc/memory/symmetric_heap.py` | 对称堆分配，peer mapping | `SymmetricHeap.create_all()` |
...
# TNCC (Tile Native Collective Communication) WG 工作组模式第 4-6 节源码解读补充

本文是对文档 [TNCC WG工作组模式完整代码流程报告.md](./TNCC%20WG%E5%B7%A5%E4%BD%9C%E7%BB%84%E6%A8%A1%E5%BC%8F%E5%AE%8C%E6%95%B4%E4%BB%A3%E7%A0%81%E6%B5%81%E7%A8%8B%E6%8A%A5%E5%91%8A.md) 中以下三节的补充解读：
