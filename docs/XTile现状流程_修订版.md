...
| 高层 op API | `xtile.ops.gemm_allscatter(...)` / `xtile.ops.allgather(...)` / `xtile.ops.reduce_scatter(...)` / `xtile.ops.gemm_reducescatter(...)` 已接入，并统一走显式 `plan` 主链 | `gemm_reducescatter(...)` 已补齐稳定 host contract，但 multiprocess 仍继承 `reduce_scatter(device)` 的 experimental gate；`full/shard` 已由 host wrapper 打通，而 `shard/full` 已确认不应继续塞进 `gemm_allscatter(...)`，应转入后续 `gemm_allgather` 风格 contract |
...
#### 4. `gemm_reducescatter(...)` 的第一版基础工作已经完成
...
#### 5. benchmark / JSON / plot / 文档导出已经开始同源
...
#### 1. allocator-first canonical backend 还没有完成
...
三层 bring-up / fallback 体系，再加上 transport-aware gate 和 support matrix。

而它还不是：

- allocator abstraction
- export/import canonical path
- external mapping / segment metadata
...
#### 2. `gemm_allgather / shard/full` 这一侧还没有形成独立公共契约

这一项只讨论 **GEMM 类高层语义**，核心是 `shard/full` 这一侧应当如何定义；它**不包含** `allreduce` 高层 contract，`allreduce` 属于下一条独立缺口。

当前已经证实：

- `full/shard` 已实现，并且高层 wrapper 已打通。
- `shard/full` 不能再被当作 `gemm_allscatter(...)` 的逆向补丁继续堆。

原因不是“代码量还不够”，而是语义本身不同：

- 当前 `gemm_allscatter_sharded(...)` 暴露的是 peer-scatter ownership contract。
- 它不是稳定的 local-shard basis，不能直接拿来做 full-output assembly。
- 因此这里缺的不是一个 wrapper，而是一个新的公共语义边界。

这一项真正未完成的内容是：

- 定义独立的 `gemm_allgather` 风格 public contract；
- 固定 local-output ownership、shape/layout contract 与 heap contract；
- 再决定复用现有 pattern，还是走“local GEMM + allgather”的高层组合路径；
- 把它和 `gemm_allscatter(...)` 的 contract 边界清楚分开，避免继续把不同语义塞进同一个 API 名字里。
...
1. **P0：allocator-first canonical backend 对齐 Iris**
   - 引入 allocator abstraction。
   - 明确 export / import / map / access 统一语义。
   - 把当前 multiprocess fallback 能力收口到底层 canonical layer。

2. **P1：把 `shard/full` 从错误挂靠里拆出来，定义独立 public contract**
   - 不再继续把它当 `gemm_allscatter(...)` 的补丁。
   - 直接按 `gemm_allgather` 风格重新定义语义和验收。

3. **P2：把 multiprocess 支持面从“局部成立”扩成“可信成立”**
   - 继续修 `pytorch_ipc` / `peer_access_pointer_exchange`。
   - 扩到 `world_size > 2`。
   - 补更大 shape、stress 和 performance 验收。

4. **P3：继续收紧 collective 与性能闭环**
