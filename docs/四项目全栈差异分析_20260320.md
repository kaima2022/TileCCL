...
- 最新 full 6-size rerun 虽然已经做到 `1.619x`，但优势并非所有尺寸都同样稳定
...
- `Tile` 还是临时 alias
- 用户还没有 `xtile.ops.gemm_allscatter(...)` 这类一键式高层入口
- pattern / collective / benchmark 虽然共用同一 runtime，但“用户默认路径”还不是高层 op API
...
旧数据里最好曾到 `1.067x`，更早一轮统一 runtime 复测里也出现过 `1.004x` 的保守结果；但在显式 contract + plan-builder 主链稳定后，当前 full 6-size rerun 的最新 headline 已经更新为 `1.619x`。
