...
- `xtile.ops.gemm_allscatter(...)`、`xtile.ops.allgather(...)`、`xtile.ops.reduce_scatter(...)`、`xtile.ops.gemm_reducescatter(...)` 现都已落地；其中 `gemm_reducescatter(...)` 当前已补齐稳定 host-side contract，但 multiprocess/public-performance gate 仍未闭环
...
【2026-03-22 修订】这一项在当前 public surface 上已经闭环：
- `xtile.ops.gemm_allscatter(...)`
- `xtile.ops.gemm_reducescatter(...)`
- `xtile.ops.allgather(...)`
- `xtile.ops.allreduce(...)`

当前更准确的说法已经不再是“P1 还缺 API”，而是：
- 当前 public multiprocess surface 已明确收口为 `world_size=2 + ctypes_ipc`
- 这组 API 在该 surface 上都已完成 plan contract、host/support/ops 回归与结构化 benchmark 验收
- 下一步缺口转为更多 transport、`world_size > 2`、stress 与 performance gate，而不是 API 名字本身
...
【2026-03-22 修订】这一项在当前 public surface 上也已经完成第一阶段闭环：
- host/support/ops 回归：`67 passed`
- multiprocess public-surface 回归：`15 passed`
- `allgather / reduce_scatter / allreduce / gemm_allgather / gemm_reducescatter / gemm_allscatter / gemm_allscatter auto patterns` 的结构化 benchmark artifact 已全部复测通过

当前剩余问题是更高阶的 P2，而不是“P2 还没开始”：
- `pytorch_ipc`
