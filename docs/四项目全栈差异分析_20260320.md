...
【2026-03-22 修订】前三项里，`gemm_allscatter(...)` / `gemm_reducescatter(...)` / `allgather(...)` 现已落地。当前这一项剩余的核心缺口已经不再是“高层 API 不存在”，而是：
- `allreduce(...)` 还没有同级别稳定高层 contract
- `gemm_reducescatter(...)` 虽已落地，但 multiprocess 仍保持 transport-aware experimental gate
