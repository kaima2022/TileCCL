# TNCC (Tile Native Collective Communication) 现状流程：GEMM + AllScatter 全栈代码流
...
当前仓库里与 TNCC 主流程直接相关的公开入口，至少包括：

- `tncc.init(...)`、`tncc.init_local(...)`、`tncc.current_context()`
- `tncc.ops.gemm_allscatter(...)`、`tncc.ops.gemm_allscatter_sharded(...)`
- `tncc.ops.gemm_allgather(...)`、`tncc.ops.gemm_reducescatter(...)`
- `tncc.ops.allgather(...)`、`tncc.ops.allreduce(...)`、`tncc.ops.reduce_scatter(...)`
- `ctx.empty(...)`、`ctx.zeros(...)`、`ctx.randn(...)`、`ctx.workspace(...)`
- `ctx.auto_select_pattern(...)` 与 `tncc.patterns.auto_select(...)`

这里有两条口径必须写死，避免后面文档继续漂移：

- `ctx.randn(...)`、`ctx.empty(...)`、`ctx.zeros(...)` 都是当前真实 API，不是示意写法。
- 仓库中不存在 `tncc.fused_gemm_scatter(...)` 这个公开名字；当前稳定主入口仍然是 `tncc.ops.gemm_allscatter(...)`。

### 实际源码（方式 A：当前推荐用户入口，高层 API）

这部分直接贴当前 `tncc/ops.py` 里的真实入口实现。当前推荐用户入口仍是 `tncc.ops.gemm_allscatter(...)`，其内部已经升级成“build plan -> execute plan”的主链：
...
除了默认 public `tncc.ops.*` 主链，当前仓库里还有两条真实但更偏 expert/internal 的使用方式：

- 可以直接 `build_gemm_allscatter_plan(...)` 后重复 `plan.execute(...)`，这不是过时残留，而是稳定可复用的 host-side plan 路径。
- 可以通过 `tncc.patterns.auto_select(...)` 或 `ctx.auto_select_pattern(...)` 拿到具体 pattern，再直接 `pattern.execute(...)`；只是它现在不再应被文档写成“默认 public 入口”。

另外，`tncc.ops.gemm_allscatter_sharded(...)` 仍然是明确存在的 expert `shard/shard` 契约，不应在文档里被省略成“只有 full/full 一条路”。

### 当前状态与边界

| 主题 | 当前状态 | 当前边界 |
|------|------|------|
| 用户入口 | `tncc.ops.gemm_allscatter(...)` 已可用 | 高层 API 命名与最终 op 集合仍可继续收敛 |
| 堆上张量分配 | `TNCCContext` 已支持 `empty/zeros/randn()` | 已打通 |
| 后端检测 | `tncc.init()` 默认就是 `backend="auto"` | 已集成 |
| runtime ctx 主路径 | `tncc.init(..., heap=...)` / `heap_size=...` / `init_local(...)` 都可直接返回可运行 pattern 的真实 ctx | 主路径已统一 |
| benchmark / tests / CLI / runtime 一致性 | 已统一到 `TNCCContext` | 已打通 |
| 高层 op API | `tncc.ops.gemm_allscatter(...)` / `tncc.ops.gemm_allgather(...)` / `tncc.ops.allgather(...)` / `tncc.ops.allreduce(...)` / `tncc.ops.reduce_scatter(...)` / `tncc.ops.gemm_reducescatter(...)` 已接入，并统一走显式 `plan` 主链 | 在当前 public multiprocess surface（`world_size=2 + ctypes_ipc`）上，上述入口已完成同级别 host contract、回归和结构化 benchmark 验收；更大 world size、更多 transport、stress 与性能门禁仍未完成 |
...
也就是说，用户表面上看到的是 `tncc.ops.*`，但真正落到设备侧仍然是“plan 解析 contract，再由 pattern 或 collective launcher 驱动 primitives 执行”。
...
因此，TNCC 当前 pattern 层不是“只有 fused kernel 一条路”，而是已经形成了可显式选择、也可 auto-select 的四类真实执行形态。

### 实际源码：显式 execution contract

当前 pattern 层的关键变化不是 kernel 花样，而是 `tncc/patterns/contracts.py` 先把逻辑 shape/layout 解释清楚，再交给 pattern 执行。下面是当前真实源码摘录：
...
- 当前仍未完全收敛的，是默认 public API 是否全部走 `tncc.ops.*`，以及是否继续保留多种输出 layout 模型

### 编译可见性补充

这部分要说得更准确一点：TNCC 当前并没有像 Triton-distributed 那样再额外引入一层 distributed dialect。

host 侧主链更接近下面这种形态：

```text
tncc.ops.* / build_*_plan(...)
  -> 校验 tensor contract
...
`tncc.current_context()` 只是返回当前 attached ctx；真正决定 symmetric heap 建立方式的，仍然是 `init_local(...)` 还是 `init(...)`。

### 模式 A：单进程 — SymmetricHeap.create_all（推荐）

```python
# tncc/memory/symmetric_heap.py
...
- `TNCC_FORCE_MULTIPROCESS_TRANSPORT=pytorch_ipc python -u -m tests.test_e2e._run_ipc_test`
  - 结果：`ALL MULTI-GPU IPC TESTS PASSED`
- `TNCC_FORCE_MULTIPROCESS_TRANSPORT=peer_access_pointer_exchange python -u -m tests.test_e2e._run_ipc_test`
...
- `TNCC_ENABLE_EXPERIMENTAL_MULTIPROCESS_DEVICE_COLLECTIVES=1 python -m tests.test_e2e._run_reduce_scatter_multiprocess`
...
- 所以它不能继续作为 TNCC multiprocess device path 的自动 fallback
...
**与 Iris 的区别**：不能再简单写成“Iris 仅使用 HIP IPC 单一路径”。当前 Iris 的主实现已经演进到 allocator + fd passing + DMA-BUF 映射；TNCC 的特点也不应再表述成“三条 transport 同等可用”。更准确的表述是：TNCC 显式实现了 `ctypes_ipc` / `pytorch_ipc` / `peer_access_pointer_exchange` 三条 bring-up 策略，并且已经通过真实矩阵确认只有 `ctypes_ipc` 可作为当前 multiprocess device path 的 auto transport。

### Iris 与 TNCC 的实质差异、优劣与对齐方向
...
| 单进程 bring-up | 不如 TNCC 直接 | 很直接，`create_all()` 非常适合单节点 benchmark |
...
- 如果问“**当前这台 H100 机器上谁更实用**”，TNCC 现在更合适，因为它已经覆盖了当前环境里的关键约束，尤其是在 `ptrace_scope=1` 这类限制下仍能跑通。

**TNCC 应该如何向 Iris 对齐，而不是简单照抄？**

总纲不变，但详细动作已经并入下文“当前差距与下一步计划”。核心原则只有三条：

1. 保留 TNCC 当前“多 transport 显式实现 + 受控强制诊断”的工程优势，但 auto path 继续只走已验证的 device-safe transport。
...
- `tncc.memory.allocators` 已建立 allocator backend 边界，默认 backend 为 `torch_bump`
...
- `import_external_tensor(...)` / `as_symmetric(...)` 与 `TNCCContext.as_symmetric(...)` 已接入，当前 external import 语义明确是 copy-based
- `tncc.describe_runtime_support(ctx)` / `ctx.support_matrix()` / `tncc support --json` 已形成统一状态出口
- `figures/data/*.json`、`scripts/plot_figures.py`、`docs/generated/benchmark_runtime_summary.md` 已开始共享同一套结构化数据源

这部分要写准确：它说明 TNCC 的 allocator-first v1 已经落地，但**不等于** canonical `allocator + export/import-map/access` backend 已经完成。
...
- 传 `ctx` 时，`auto_select(...)` 返回“已绑定上下文的 pattern 实例”；不传 `ctx` 时，返回 pattern 类本身。这意味着它既能用于 `tncc.ops.*` 内部，也能用于 expert 手工持有 pattern 的路径。
...
当前 TNCC 已经进入 allocator-first v1，但离 Iris 风格的 canonical `allocator + export/import-map/access` 统一底座还有实质差距。
...
- TNCC 当前优势在显式 contract、显式 support matrix、显式 feature gate 与诊断透明度
- TNCC 不应简单照抄 Iris；正确方向是保留现在这套可观测和可诊断的工程外壳，同时把底层逐步收口到 canonical allocator/import-map/access
...
当前更稳妥的结论是：TNCC 已经具备继续工程化推进的基础，但距离理想通信库仍差 canonical substrate、broader support surface，以及完整的性能与生产级验证闭环。

## 与 NCCL 的差距归因及未来方向

这一章单独回答一个更尖锐、也更实际的问题：**如果把 TNCC 当成“独立通信库”来衡量，它和 NCCL 现在到底差在哪里，下一步应当怎么补。**
...
这里必须特别说明：当前这组数据代表的是“**今天真实可部署、真实可回归的 TNCC collective 层**”，不是“理论上如果把内部 fast path 全部写满后可能达到的上限”。

### 当前结果

在当前 compact canonical run 中，5 个 communication-only collective 的最佳点如下：

| collective | 最佳消息尺寸 | TNCC | NCCL | TNCC / NCCL |
|------|------|------|------|------|
...
### 补充内部口径：TNCC vs `bulk_sync`

如果当前目标不是和 NCCL 正面对标，而是向项目申请材料说明“TNCC 的 collective-specific kernel 相比 naive internal baseline 到底带来了什么”，那么更合适的补充口径是：
...
- 它**不是**“TNCC 已经超过外部成熟通信库”的证据。
- 它是“在 TNCC 自己的通信栈内部，collective-specific kernel 相对 naive bulk_sync baseline 的结构性收益”。
- 这更适合说明：TNCC 已经不只是“能通信”，而是开始具备“专用 collective kernel 的体系化收益”。
...
### TNCC / bulk_sync / NCCL 端到端执行流程对比

| 阶段 | `TNCC` | `bulk_sync` | `NCCL` |
|------|------|------|------|
...
| 调度入口 | TNCC collective primitive / launcher | host 侧 phase-by-phase 编排 | NCCL collective API |
...
| 工程控制面 | kernel、heap、pointer translation、runtime glue 都在 TNCC 内部 | phase、copy、stage buffer、同步都在 TNCC host 侧显式控制 | 算法、协议、通道、调度都在 NCCL 内部 |
...
- 如果是说明“TNCC 相比成熟通信库还有多远”，看 `fig6_collective_comm_only.png`。
- 如果是说明“TNCC 当前专用 collective kernel 已经比 naive internal baseline 好在哪里”，看 `fig7_collective_bulk_sync.png`。

### 差距归因

#### 1. `allreduce` 已经完成单一路径收口，但仍不是成熟高性能 collective

当前 `tncc.primitives.collectives.allreduce(...)` 的默认 public path 已经不再是 `reduce_scatter + allgather + copy` 的 host-side 组合，而是显式 resolve execution spec 后直接进入 staged device path：
...
这也是 TNCC 与 NCCL、以及与 Iris 当前更偏 allocator-first / import-map 的路线之间的核心差距之一。
...
- 即使只看 `64 KiB` 这个当前 best point，TNCC 的 `allreduce` 仍然只有 `0.11x NCCL`
...
如果只从“TNCC 是不是已经是一套接近 NCCL 的通信库”来问，当前答案必须是：

- **还不是。**

更细一点地说：

- TNCC 的 **P2P substrate** 与 **GEMM 基础路径** 并不弱，至少已经形成了可继续工程化推进的底子。
...
- 让 `tncc.ops.allreduce(...)` 和 `tncc.primitives.collectives.allreduce(...)` 统一到同一条 launcher 主链
...
1. **先固定 public 目标形态**
   - `tncc.ops.allreduce(...) -> build_allreduce_plan(...) -> plan.execute(...)` 这一层稳定入口可以保持不变。
   - 这一步现在已经完成：`tncc.primitives.collectives.allreduce(...)` 的默认 public path 已经切到 staged single-path launcher。
...
- 保留 TNCC 当前 `support matrix + feature gate + diagnostics` 的工程透明度
