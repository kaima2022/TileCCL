...
"""Real multiprocess validation for gemm_allgather."""
...
def test_gemm_allgather_multiprocess_default_transport(
    skip_no_multigpu,
    dtype_name: str,
) -> None:
    """The default multiprocess gemm_allgather path should be correct on the current runtime."""
    ...
    if result.returncode != 0:
        raise AssertionError(
            "Multiprocess gemm_allgather validation failed.\n"
            f"stdout:\n{result.stdout}\n"
            f"stderr:\n{result.stderr}"
        )
