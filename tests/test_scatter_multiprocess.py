...
"""Real multiprocess validation for scatter."""
...
@pytest.mark.parametrize(
    ...
    [
        ("float16", 128, "legacy", "legacy", "flat_root_scatter"),
        ("bfloat16", 128, "legacy", "legacy", "flat_root_scatter"),
        ("float32", 128, "legacy", "legacy", "flat_root_scatter"),
        ("float32", 4097, "staged", "staged", "ws2_slot_epoch_pipeline"),
    ],
)
def test_scatter_multiprocess_default_transport(
    skip_no_multigpu,
    dtype_name: str,
    block_size: int,
    expected_path: str,
    ...
    """Scatter should be correct for root=0/1 across legacy and staged sizes."""
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "tests.test_e2e._run_scatter_multiprocess",
            "--dtype",
            dtype_name,
            "--block-size",
            str(block_size),
    ...
    if result.returncode != 0:
        raise AssertionError(
            "Multiprocess scatter validation failed.\n"
            f"stdout:\n{result.stdout}\n"
            f"stderr:\n{result.stderr}"
        )
