...
def test_support_matrix_without_heap(skip_no_gpu, device_info) -> None:
    ...
    assert matrix.ops["allreduce"].state == "partial"
    ...
    assert matrix.contracts["allreduce.in_place"].state == "partial"
    ...
    assert matrix.memory["symmetric_heap.external_memory_interface"].state == "partial"
    assert matrix.memory["symmetric_heap.segment_metadata"].state == "partial"
    assert matrix.memory["symmetric_heap.segment_layout"].state == "partial"
    assert matrix.memory["symmetric_heap.exportable_segment_metadata"].state == "partial"
    ...
    assert matrix.memory["symmetric_heap.peer_segment_catalog"].state == "partial"
...
def test_support_matrix_with_heap_matches_context_method(
    ...
    try:
        ...
        assert direct.ops["allreduce"].state == "supported"
        ...
        assert direct.contracts["allreduce.in_place"].state == "supported"
        ...
        assert direct.collectives["collectives.allreduce_launcher"].state == "supported"
        ...
        assert direct.memory["symmetric_heap.external_memory_interface"].state == "supported"
        assert direct.memory["symmetric_heap.segment_metadata"].state == "supported"
        assert direct.memory["symmetric_heap.segment_layout"].state == "supported"
        assert direct.memory["symmetric_heap.exportable_segment_metadata"].state == "supported"
        ...
        assert direct.memory["symmetric_heap.peer_segment_catalog"].state == "supported"
...
def test_support_matrix_multigpu_reports_peer_access(
    ...
    try:
        ...
        assert matrix.collectives["collectives.allreduce_launcher"].state == "supported"
        ...
        assert matrix.memory["symmetric_heap.external_memory_interface"].state == "supported"
        assert matrix.memory["symmetric_heap.segment_metadata"].state == "supported"
        assert matrix.memory["symmetric_heap.segment_layout"].state == "supported"
        assert matrix.memory["symmetric_heap.exportable_segment_metadata"].state == "supported"
        ...
        assert matrix.memory["symmetric_heap.peer_segment_catalog"].state == "supported"
...
def test_support_matrix_multiprocess_validated_surface_is_supported(
    skip_no_gpu,
    device_info,
) -> None:
    """The validated 2-GPU ctypes_ipc surface should now be public-supported."""
    ...
    assert matrix.ops["allgather"].state == "supported"
    assert matrix.ops["allreduce"].state == "supported"
    assert matrix.ops["gemm_allscatter"].state == "supported"
    assert matrix.ops["gemm_allgather"].state == "supported"
    assert matrix.ops["gemm_reducescatter"].state == "supported"
    assert matrix.execution_paths["reduce_scatter.reference"].state == "unsupported"
    assert matrix.execution_paths["reduce_scatter.device"].state == "supported"
    assert matrix.memory["symmetric_heap.device_remote_access"].state == "supported"


def test_support_matrix_multiprocess_opt_in_remains_partial_outside_public_surface(
    skip_no_gpu,
    device_info,
    monkeypatch,
) -> None:
    """Broader world-size diagnostics may still run under the explicit opt-in."""
