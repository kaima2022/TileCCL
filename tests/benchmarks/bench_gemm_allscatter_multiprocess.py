#!/usr/bin/env python3
"""Structured multiprocess gemm_allscatter matrix."""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-json",
        type=str,
        default=str(Path("docs/generated/gemm_allscatter_multiprocess_matrix.json")),
...
def _aggregate_rank_payloads(payloads: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "pattern": payloads[0]["pattern"],
        "plan_pattern_name": payloads[0]["plan_pattern_name"],
...
def main() -> None:
    """Run the structured multiprocess GEMM + all-scatter matrix."""
        ...
        for dtype_name in dtypes:
            for transport in transports:
                ...
                try:
                    ...
                    print(
                        f"[FAIL] contract={contract:10s} dtype={dtype_name:8s} "
                ...
                if payload["status"] == "passed" and isinstance(summary, dict):
                    print(
                        f"[PASS] contract={contract:10s} dtype={dtype_name:8s} "
                        f"requested={transport:28s} actual={summary['transport_strategy']:28s} "
    ...
    payload = {
        "schema_version": 1,
        "benchmark": "gemm_allscatter_multiprocess_matrix",
