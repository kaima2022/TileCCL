...
"""Structured multiprocess Triton remote-access transport matrix."""
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--block-size", type=int, default=128, help="Block size per case.")
    parser.add_argument(
        "--timeout-sec",
        type=int,
        default=180,
    ...
    parser.add_argument(
        "--output-json",
        type=str,
        default=str(Path("docs/generated/triton_remote_access_multiprocess_matrix.json")),
...
def _aggregate_rank_payloads(
    ...
    *,
    ...
    if operation in {"load", "both"}:
        load_means = [
            float(item["load_timing_ms"]["mean_ms"])  # type: ignore[index]
            for item in payloads
        ]
        summary["load_mean_ms_across_ranks"] = statistics.mean(load_means)
        summary["load_max_rank_skew_ms"] = abs(max(load_means) - min(load_means))
    if operation in {"store", "both"}:
        store_means = [
            float(item["store_timing_ms"]["mean_ms"])  # type: ignore[index]
            for item in payloads
        ]
        summary["store_mean_ms_across_ranks"] = statistics.mean(store_means)
        summary["store_max_rank_skew_ms"] = abs(max(store_means) - min(store_means))
    return summary


def main() -> None:
    """Run the structured multiprocess Triton remote-access matrix."""
    ...
    for dtype_name in dtypes:
        for transport in transports:
            cmd = [
                ...
                "-u",
                "-m",
                "tests.test_e2e._run_triton_remote_access_multiprocess",
                "--dtype",
                dtype_name,
                "--warmup",
                str(args.warmup),
            ...
            if result.returncode == 0:
                ...
                payload["summary"] = _aggregate_rank_payloads(
                    rank_payloads,
                    operation=args.operation,
                )
            else:
            ...
            if payload["status"] == "passed" and isinstance(summary, dict):
                details: list[str] = []
                if args.operation in {"load", "both"}:
                    details.append(f"load={summary['load_mean_ms_across_ranks']:.3f} ms")
                if args.operation in {"store", "both"}:
                    details.append(f"store={summary['store_mean_ms_across_ranks']:.3f} ms")
                print(
    ...
    payload = {
        "schema_version": 1,
        "benchmark": "triton_remote_access_multiprocess_matrix",
