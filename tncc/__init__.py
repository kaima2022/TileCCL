...
if TYPE_CHECKING:
    ...
    from tncc.backends.base import BackendInterface, TopologyInfo
    from tncc.memory.symmetric_heap import SymmetricHeap


# ---------------------------------------------------------------------------
# TNCCContext -- returned by init()
# ---------------------------------------------------------------------------
...
class TNCCContext:
    ...
    def require_heap(self) -> "SymmetricHeap":
        ...
        if self.heap is None:
            raise RuntimeError(
                "No SymmetricHeap is attached to this TNCCContext. "
                "Pass heap=... or heap_size=... to tncc.init(), call "
                "tncc.init_local(...), or attach an existing heap via "
                "ctx.attach_heap(heap)."
            )
        return self.heap

    def attach_heap(self, heap: "SymmetricHeap") -> "TNCCContext":
        """Attach an existing :class:`~tncc.memory.symmetric_heap.SymmetricHeap`.
    ...
    def support_matrix(self):
        ...
        from tncc.support import describe_runtime_support
...
_ctx: Optional[TNCCContext] = None


def _get_ctx() -> TNCCContext:
    """Return the global context, raising if :func:`init` was not called."""
    if _ctx is None:
        raise RuntimeError(
            "TNCC has not been initialised. Call tncc.init() first."
        )
    return _ctx


def describe_runtime_support(ctx: TNCCContext | None = None):
    """Return the current structured runtime support matrix."""
    from tncc.support import describe_runtime_support as _describe_runtime_support
...
def init(
    ...
    *,
    ...
    """Initialize the TNCC runtime.

    Detects (or accepts) the GPU backend, rank, and world size, then
    returns an :class:`TNCCContext` that all subsequent API calls use.
    ...
    Args:
        ...
        heap: Existing symmetric heap to attach to the returned context.
            ...
            :meth:`tncc.memory.symmetric_heap.SymmetricHeap.create_all`.
    ...
    Returns:
        An :class:`TNCCContext` carrying rank, world_size, device, backend,
...
def current_context() -> TNCCContext:
...
def _build_context(
    *,
    ...
    from tncc.backends import get_backend
...
def _detect_topology_safe(backend: str) -> Optional["TopologyInfo"]:
    ...
    try:
        from tncc.utils.topology import detect_topology
...
def _create_heap_for_context(ctx: TNCCContext, heap_size: int) -> "SymmetricHeap":
    ...
    raise RuntimeError(
        ...
        "to be initialised. For single-process multi-GPU, use tncc.init_local(...); "
...
# Tile is a forward-reference alias; the real class lives in tncc.memory.
# We defer import to avoid circular dependencies at startup.
def __getattr__(name: str):
    """Lazy import for heavy sub-modules to keep ``import tncc`` fast."""
    if name == "Tile":
        # Placeholder alias -- will resolve to a proper Tile class once
        # tncc.memory.tile is implemented.
        from tncc.memory.symmetric_heap import SymmetricHeap as _SH
    ...
    if name == "patterns":
        ...
        _patterns = importlib.import_module("tncc.patterns")
        return _patterns
    if name == "ops":
        import importlib

        _ops = importlib.import_module("tncc.ops")
        return _ops
    raise AttributeError(f"module 'tncc' has no attribute {name!r}")
