...
"""
tileccl.backends - Hardware Abstraction Layer (HAL).

This package abstracts over NVIDIA (CUDA) and AMD (HIP) GPUs so that the
rest of TileCCL can be written in a hardware-agnostic way.
...
def get_backend(backend: Optional[str] = None, *, force: bool = False) -> BackendInterface:
    ...
    if backend == "cuda":
        from tileccl.backends.cuda import CUDABackend
    ...
    if backend == "hip":
        from tileccl.backends.hip import HIPBackend
