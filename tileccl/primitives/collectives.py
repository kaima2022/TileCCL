...
"""
tileccl.primitives.collectives - Collective communication primitives.

Tile-level collective operations implemented entirely in Triton
(pure ``@triton.jit``, no NCCL / RCCL dependency).  All remote memory
access goes through :func:`~tileccl.memory.translation.translate_ptr`.
...
if TYPE_CHECKING:
    from tileccl.memory import SymmetricHeap
...
def _validate_allreduce_launch_prereqs(
    ...
    *,
    ...
    _require_device_remote_access_transport(
        heap,
        operation="tileccl.primitives.allreduce(...)",
    )
