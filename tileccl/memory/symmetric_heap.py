...
"""
tileccl.memory.symmetric_heap - Symmetric memory heap for multi-GPU communication.
...
from tileccl.backends import detect_hardware, get_backend
...
def _resolve_backend(backend: str) -> tuple[str, BackendInterface]:
    ...
    if backend == "auto":
        ...
        if hw == "none":
            raise RuntimeError(
                "No GPU backend detected. TileCCL requires CUDA or HIP (ROCm)."
            )
        backend = hw
