...
from tileccl.patterns.contracts import PatternExecutionSpec, resolve_pattern_execution
from tileccl.patterns.runtime import (
...
if TYPE_CHECKING:
    from tileccl.primitives.collectives import AllReduceExecutionSpec
...
class AllGatherPlan:
    ...
    def execute(self, src: Any, output: Any, *, validate: bool = True) -> Any:
        ...
        from tileccl.primitives.collectives import allgather as collective_allgather
...
class AllReducePlan:
    ...
    def execute(self, tensor: Any, *, validate: bool = True) -> Any:
        ...
        from tileccl.primitives.collectives import allreduce as collective_allreduce
...
class ReduceScatterPlan:
    ...
    def execute(self, src: Any, output: Any, *, validate: bool = True) -> Any:
        ...
        if validate:
            self.validate_tensors(src, output)
        from tileccl.primitives.collectives import reduce_scatter as collective_reduce_scatter
...
def build_gemm_allscatter_plan(
    ...
    C: Any,
    *,
    ctx: tileccl.TileCCLContext | None = None,
    ...
    """
    resolved_ctx = ctx if ctx is not None else tileccl.current_context()
...
def build_gemm_allgather_plan(
    ...
    *,
    ...
    _require_device_remote_access_runtime(
        resolved_ctx,
        operation="tileccl.ops.gemm_allgather(...)",
    )
    contract = _resolve_gemm_allgather_contract(
        A,
        B_shard,
...
def build_allgather_plan(
    ...
    *,
    ...
    _require_device_remote_access_runtime(
        resolved_ctx,
        operation="tileccl.ops.allgather(...)",
    )
...
def build_allreduce_plan(
    ...
    *,
    ...
    _require_allreduce_runtime(
        resolved_ctx,
        operation="tileccl.ops.allreduce(...)",
    )
    ...
    from tileccl.primitives.collectives import resolve_allreduce_execution
...
def _require_gemm_allscatter_runtime(ctx: tileccl.TileCCLContext) -> None:
    """Validate the runtime prerequisites for GEMM + all-scatter execution."""
    _require_device_remote_access_runtime(
        ctx,
        operation="tileccl.ops.gemm_allscatter(...)",
    )
...
def _require_tensor_on_heap(tensor: Any, *, ctx: tileccl.TileCCLContext, name: str) -> None:
...
def build_allgather_gemm_reducescatter_plan(
    ...
    *,
    ...
    _require_device_remote_access_runtime(
        resolved_ctx,
        operation="tileccl.ops.allgather_gemm_reducescatter(...)",
    )
    contract = _resolve_ag_gemm_rs_contract(
