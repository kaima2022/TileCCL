...
"""tileccl.cli - Command-line interface for TileCCL (Tile Native Collective Communication).

Provides the ``tileccl`` console entry point for hardware diagnostics and
benchmarking.

Usage::

    tileccl info                    # print hardware / topology info
    tileccl bench p2p               # P2P bandwidth sweep
    tileccl bench collective        # collective bandwidth benchmark
    tileccl bench pattern           # pattern overlap efficiency
    tileccl bench gemm              # GEMM vs torch.matmul
    tileccl bench all               # run all benchmarks
    tileccl bench --pattern auto    # auto-select pattern benchmark
"""
...
def _handle_info(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(f"[tileccl info] Error: {exc}", file=sys.stderr)
        sys.exit(1)
...
def _format_support_matrix(matrix) -> str:
    ...
    lines: list[str] = []
    lines.append("=== TileCCL Runtime Support Matrix ===")
    lines.append(
        "Context: "
...
def _handle_support(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(f"[tileccl support] Error: {exc}", file=sys.stderr)
        sys.exit(1)

    try:
        matrix = tileccl.describe_runtime_support(ctx)
...
def _ensure_gpu():
    ...
    try:
        ...
        print("[tileccl bench] PyTorch is required. Install with: pip install torch", file=sys.stderr)
        sys.exit(1)
    if not torch.cuda.is_available():
        print("[tileccl bench] No GPU detected. Benchmarks require a CUDA or ROCm GPU.", file=sys.stderr)
        sys.exit(1)
    return torch
...
def _ensure_multi_gpu(torch_mod, min_gpus: int = 2):
    ...
    if torch_mod.cuda.device_count() < min_gpus:
        print(f"[tileccl bench] This benchmark requires >= {min_gpus} GPUs. "
              f"Found {torch_mod.cuda.device_count()}.", file=sys.stderr)
        sys.exit(1)


def _results_dir() -> Path:
    """Return ~/.tileccl/benchmark_results/, creating it if needed."""
    d = Path.home() / ".tileccl" / "benchmark_results"
...
def _project_root() -> str:
    """Return the TileCCL project root directory."""
...
def _bench_p2p(args: argparse.Namespace) -> None:
    ...
    print("=== TileCCL P2P Bandwidth Benchmark ===")
...
def _bench_collective(args: argparse.Namespace) -> None:
    ...
    print("=== TileCCL Collective Bandwidth Benchmark ===")
    print(f"GPUs: {torch.cuda.get_device_name(0)} x {torch.cuda.device_count()}")
    print()
...
def _bench_pattern(args: argparse.Namespace) -> None:
    ...
    print("=== TileCCL Pattern Overlap Benchmark ===")
    print(f"GPUs: {torch.cuda.get_device_name(0)} x {torch.cuda.device_count()}")
    print()
...
def _bench_all(args: argparse.Namespace) -> None:
    ...
    print("=" * 70)
    print("  TileCCL Full Benchmark Suite")
    print("=" * 70)
...
def _handle_bench(args: argparse.Namespace) -> None:
    ...
    print(f"TileCCL Benchmark: pattern={pattern}, M={M}, N={N}, K={K}")
    print("-" * 60)

    from tileccl.kernels.gemm import gemm
    from tileccl.utils.profiling import TileProfiler, format_benchmark_table
...
def _run_pattern_bench(M: int, N: int, K: int, pattern: str) -> None:
    ...
    import tileccl
    from tileccl.patterns.auto_select import auto_select, benchmark_all_patterns

    world_size = 2
    heap_size = max(512 * 1024 * 1024, M * K * 2 + K * (N // world_size) * 2 + M * N * 2)
    contexts = tileccl.init_local(world_size=world_size, heap_size=heap_size)
...
def _handle_compare(args: argparse.Namespace) -> None:
    ...
    if not files:
        print("No benchmark results found in ~/.tileccl/benchmark_results/")
        return
...
def main() -> None:
    """TileCCL CLI -- benchmark and diagnostics tool."""
    parser = argparse.ArgumentParser(
        prog="tileccl",
        description="TileCCL (Tile Native Collective Communication): cross-platform tile communication library CLI",
    )
    parser.add_argument(
...
def _get_version() -> str:
    ...
    try:
        from tileccl import __version__
        return __version__
    except Exception:
        return "unknown"
